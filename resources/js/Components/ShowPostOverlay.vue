...
<template>
            ...
            <div class="w-full md:flex h-full overflow-auto rounded-xl">
                ...
                <div class="md:max-w-[500px] w-full relative">
                    <div class="flex items-center justify-between p-3 border-b">
                        ...
                        <button v-if="user.id === post.user.id" @click="deleteType = 'Post'; id = post.id">
                    ...
                    <div class="overflow-y-auto h-[calc(100%-170px)]">
                        ...
                        <div v-if="post.comments" v-for="comment in post.comments" :key="comment" class="p-3">
                            <div class="flex items-center justify-between">
                                ...
                                <DotsHorizontal v-if="user.id === comment.user.id" class="cursor-pointer"
                                    @click="deleteType = 'Comment'; id = comment.id" :size="27" />
                            </div>
                    ...
                    <LikesSection v-if="post" class="px-2 border-t mb-2" :post="post" @like="$emit('updateLike', $event)" />
                    ...
                    <div class="absolute flex border bottom-0 w-full max-h-[200px] bg-white overflow-auto">
                        ...
                        <textarea ref="textarea" :oninput="textareaInput" v-model="comment" placeholder="Add a comment..."
                            rows="1" class="
                                                            ...
                                                            text-[18px]
                                                        "></textarea>
                        <button v-if="comment" class="text-blue-600 font-extrabold pr-4" @click="
    ...
    <ShowPostOptionsOverlay v-if="deleteType" :deleteType="deleteType" :id="id" @deleteSelected="
        $emit('deleteSelected', {
